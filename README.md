...
The diagram flows top-to-bottom in five color-banded stages: four typed-dataclass design pillars + a workload point (blue/orange) feed into cross-spec validation and per-device sharding (purple), which fan out into four independent cost branches (yellow — memory, compute, collective comm, stack overheads), assembled through three overlap-aware composition equations (pink — roofline, compute/comm overlap under ρ_comm, kernel-launch and host overlap under ρ_kernel / ρ_seq with pipeline bubble γ_pp), producing TPOT / TPS / TTPS and the closed-form diagnostic bounds (light blue + green dashed — $B^*$, $\mathrm{BW_\mathrm{link}^*}$, $m_\mathrm{INC}$).

Every block corresponds to a function in `llm_perf/core/decode_model.py` (or `core/primitives/` for the shared physics); every equation has its derivation in **[`decode.md`](documentation/modeling/decode.md)** — the most important modeling doc to read, since every Pareto sweep and the entire `InferenceCalculator` path go through it, and prefill / e2e / kv paging all build on its conventions.
