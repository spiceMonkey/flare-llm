...
> Repo: `flare-llm` · Python import name: `llm_perf` (package rename deferred).
...
![decode modeling flow: 5 specs → compose check + optional optimizer → sharding factors → 5 per-device branches → 2-stage overlap assembly → TPOT](assets/decode_modeling_flow.png)

The diagram (source: [`assets/mermaid/decode_modeling_flow.mmd`](assets/mermaid/decode_modeling_flow.mmd)) is organized top-to-bottom in seven color-banded stages; each row of the diagram corresponds to one numbered step below.

1. **Five-spec inputs (blue).** `LlmModelSpec` (architecture) × `SystemSpec` (cluster hardware + per-tier fabric capability incl. `inc` disclosure) × `PartitionSpec` (`PP × TP × EP × SP`; `DP = N / product` inferred) × `TuningSpec` (workload knobs — `S`, `B`, gains, memory placement) × `FrameworkSpec` (stack-axis decisions — attention_mode, tp_ep_layout, algorithm choices, `inc_enabled`, kernel-launch budget, ρ_SW, ρ_comm, MoE A2A pattern). Partition is constructed inline at the call site; the other four come from the JSON database.
2. **Cross-spec validation (orange).** `compose_check(partition, framework)` enforces invariants like `tp_ep_layout="co_located" ⇒ attention_mode="dp" ∧ TP == EP` on calculator entry — these mix-and-match constraints can't live on a single spec.
3. **(Optional) collective-algorithm resolver (purple, dashed).** When `framework` ships `"auto"` algorithm fields (per-stack JSONs do), `optimize_collective_algorithms(model, partition, system, tuner, framework)` resolves them per (phase × collective × G × M) by `argmin(cost)`, short-circuiting to INC when the fabric advertises capability AND `framework.inc_enabled=True`. Returns a new `FrameworkSpec` with auto fields concrete. `FrameworkSpec.default()` ships `"ring"` everywhere and skips this pass.
4. **Per-device sharding factors (green).** The (partition, framework) join resolves to abstract sharding factors `D_attn`, `D_kv`, `D_exp`, `N_replica` (`core/primitives/sharding_factors.py`). These encode every production-relevant configuration (orthogonal TP+EP, co-located TP+EP, DP-attention, MLA) in one lookup table so the downstream rooflines stay framework-agnostic.
5. **Five per-device branches (yellow).** Four cost rooflines plus one data precursor, computed independently from the sharding factors + tuner + framework:
    - `compute_flops` → $F_\mathrm{token}$, $F_\mathrm{step}$ → $t_\mathrm{compute} = F_\mathrm{step}/R_\mathrm{GPU}$, Tensor-Core-efficiency-derated.
    - `compute_traffic` → per-tier $T_\theta$, $T_\mathrm{KV}$, $T_\mathrm{token,eff}$ (data; feeds the memory roofline via the dashed edge).
    - `compute_memory` → $t_\mathrm{mem}(B) = \sum_i (T_{\theta,i} + B \cdot T_\mathrm{KV,i})/\mathrm{BW_\mathrm{eff,i}}$ — multi-tier roofline; single-tier devices reduce to $T_\mathrm{step}/\mathrm{BW_\mathrm{HBM}}$ bit-for-bit.
    - `compute_comm` → $t_\mathrm{PP}$, $t_\mathrm{TP}$, $t_\mathrm{EP}$, $t_\mathrm{SP}$ via `cost_collective` dispatch (SW α-β or INC short-circuit, per fabric capability and `framework.inc_enabled`).
    - **kernel-launch dispatch** → $t_\mathrm{SW} = \tau_\mathrm{launch} \cdot K(\text{partition})$, where the kernel count $K = (L/PP)(k_c + k_\mathrm{coll}(n_\mathrm{TP}+n_\mathrm{SP})) + (L_\mathrm{moe}/PP) \cdot k_\mathrm{coll} \cdot n_\mathrm{EP} + k_\mathrm{pp\_hop}$ depends on partition shape and per-layer collective count.
6. **Two-stage overlap assembly (pink).** HW-side first: $t_\mathrm{local} = \max(t_\mathrm{compute,eff}, t_\mathrm{mem})$ picks the bottleneck side of the roofline, then $t_\mathrm{stage,hw} = t_\mathrm{local} + \max(0, t_\mathrm{comm} - \rho_\mathrm{comm} \cdot t_\mathrm{local})$ folds in collective comm under the HW overlap fraction $\rho_\mathrm{comm}$. SW-side second: $t_\mathrm{stage} = t_\mathrm{stage,hw} + \max(0, t_\mathrm{SW} - \rho_\mathrm{SW} \cdot t_\mathrm{stage,hw})$ folds in the CUDA-launch budget under $\rho_\mathrm{SW}$.
7. **Final step time (red).** Pipeline bubble $\gamma_\mathrm{pp} = \max(1, PP/B)$, the LM-head one-shot $t_\mathrm{LM,hw}(B)$ on stage `PP−1`, and the per-sequence serving overhead $t_\mathrm{serving}$ join the assembly: $t_\mathrm{step,user}(B) = \gamma_\mathrm{pp} \cdot t_\mathrm{stage} + t_\mathrm{LM,hw} + t_\mathrm{serving}$.
8. **User-observed outputs (light blue).** $\mathrm{TPOT} = t_\mathrm{step,user}$; Interactivity $= 1 / \mathrm{TPOT}$ (per-user tok/s); cluster throughput $\mathrm{TTPS} = \mathrm{DP} \cdot B / t_\mathrm{step,user}$; memory→compute crossover $B^* = T_\theta \cdot R / (F_\mathrm{token} \cdot \mathrm{BW_\mathrm{eff,0}} - T_\mathrm{KV} \cdot R)$.
