...
2. **TP+EP co-location.** A layout-level choice: TP and EP map to the *same* physical GPUs of a replica rather than disjoint GPU sets (DeepSeek-V3 / R1 production decode on SGLang and NVIDIA Dynamo). With both axes overlapped on the same `max(TP, EP)` GPUs, the TP group and the EP group coincide physically. Attention can run in either mode on this overlay: under DP-attn the attention block replicates and shards the batch across the group; under TP-attn attention head-shards across the same group that holds the experts (the production DSr1 / NVL72 panel-(b) pattern with TP=EP=8 on 8 GPUs, attention head-sharded into 8 head-shards). Expert weights are no longer further TP-sharded within an expert-owning rank — each owned expert sits whole on its host GPU.
...
- $TP$ — Tensor Parallelism. Sharding factor for matrix multiplies (Megatron-LM column/row parallel). Under orthogonal layout, TP picks one GPU per attention/FFN shard; under co-location, TP and EP overlap on the same physical GPUs and attention can run in either mode on that overlay (attention head-sharded across the group under TP-attn, or replicated and batch-sharded under DP-attn — the per-device divisors are encoded in $D_{\text{attn}}$, $D_{\text{exp}}$ from §1).
...
## 19. Per-Step Serving Runtime Overhead
_(→ decode.md §7.3)_

Captures host-side per-step work — both the per-sequence inner loops (block-table gather, sampling glue, etc.) and the B-independent orchestration floor (CUDA-Graph launch decision, scheduler tick, output framing, sampler glue between graph replays). Distinct from the kernel-launch dispatch budget $t_{\mathrm{stage,kernel}}$ (per microbatch, near-constant in $B$; `decode.md §7.1`) and from the per-request scheduler latency $t_{\mathrm{sched}}$ of §13 above (once per request, not once per step). Symbols introduced in `decode.md §7.3`.

- $c_{\mathrm{orch}}$ — Per-step orchestration constant (seconds/step). B-independent host floor: CUDA-Graph launch decision, scheduler tick, output framing, sampler glue that fires between graph replays. Default 0 in the framework. Empirically the dominant source of residual under-prediction in §5 panel-(d) DP-attention large-B regimes.
...
- $t_{\mathrm{step,seq}}(B) = c_{\mathrm{orch}} + c_{\mathrm{seq}} \cdot B$ — Aggregate per-step gross host work. Composed against the per-step hardware window $t_{\mathrm{step,base}}(B)$ via the overlap factor $\rho_{\mathrm{seq}}$ in `decode.md §7.3`; sits outside the pipeline-bubble multiplier $\gamma_{\mathrm{pp}}$ since both components fire once per step on the head node regardless of bubble depth.

When $c_{\mathrm{orch}} = c_{\mathrm{seq}} = 0$ (the default), $t_{\mathrm{step,seq}}(B) = 0$ and the user-observed step time collapses to the hardware-only roofline of §7.2 — both terms are opt-in.
