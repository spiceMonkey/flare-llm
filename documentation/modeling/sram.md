...
The constant per-tier $\eta_{\beta,i}$ above captures the average sustained / nameplate ratio. For decode workloads with very large active-sequence counts $B$, HBM additionally exhibits a $B$-dependent saturation — bank-conflict rate, controller-queue depth, and KV-page-table traffic all rise with $B$ — which can be modeled as a multiplicative curve $\eta_\beta(B)$ on top of the constant per-tier deflator: $BW_{\mathrm{eff},i}(B) = BW_i \cdot \eta_{\beta,i} \cdot \eta_\beta(B)$. The mechanics, anchor values, and operating regimes where this curve becomes load-bearing are documented in `decode.md §6.2`; the symbol register lives in `notation.md §20`. For the SRAM-resident workloads in this document's scope, $\eta_\beta(B) \equiv 1$ is the operating assumption.
